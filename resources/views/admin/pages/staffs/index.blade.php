    ...
    <div class="body">
        <div class="table-responsive">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                <div class="row">
                    <div class="col-sm-6">
                        <a type="button" class="btn bg-brown btn-lg waves-effect" href="{{ route('staff.create') }}">
                            Thêm
                        </a>
                    </div>
                    <!-- <div class="col-sm-6">
                        ...
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            ...
                            @foreach($data as $data)
                                <tr role="row" class="odd">
                                    <td>{{ $data->ten_nv }}</td>
                                    <td>{{ TenChucVu($data->id_cvu) }}</td>
                                    <td>{{ $data->diachi_nv }}</td>
                                    <td>{{ $data->phone_nv }}</td>
                                    <td>{{ $data->cmnd_nv }}</td>
                                    <td>
                                        <a type="button" class="btn bg-blue-grey waves-effect" href="{{ route('staff.edit', ['data' => $data->id]) }}">
                                            <i class="material-icons">edit</i>
                                        </a>
                                    </td>
                                    <td>
                                        <div data-toggle="modal" data-target="#cfmDel" onclick="$('#formDelete').attr('action', '{{ route('staff.destroy', ['data' => $data->id]) }}');" class="btn bg-red waves-effect">
