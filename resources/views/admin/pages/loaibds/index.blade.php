                        ...
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            ...
                            @foreach($data as $data)
                                <tr role="row" class="odd">
                                    <td>{{ $data->id }}</td>
                                    <td>{{ $data->ten_loaibds }}</td>
                                    <td>{{ $data->ghichu }}</td>
                                    <td>
                                        <a type="button" class="btn bg-blue-grey waves-effect" href="{{ route('loaibds.edit', ['data' => $data->id]) }}">
                                            <i class="material-icons">edit</i>
                                        </a>
                                    </td>
                                    <td>
                                        <div data-toggle="modal" data-target="#cfmDel" onclick="$('#formDelete').attr('action', '{{ route('loaibds.destroy', ['data' => $data->id]) }}');" class="btn bg-red waves-effect">
