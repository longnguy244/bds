    ...
    <div class="body">
        <div class="table-responsive">
            <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                <div class="row">
                    <div class="col-sm-6">
                        <a type="button" class="btn bg-brown btn-lg waves-effect" href="{{ route('ward.create') }}">
                            Thêm
                        </a>
                    </div>
                </div>
                        ...
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            ...
                            @foreach($data as $data)
                                <tr role="row" class="odd">
                                    <td>{{ $data->id }}</td>
                                    <td>{{ $data->id_quan }}</td>
                                    <td>{{ $data->ten_phuong }}</td>
                                    <td>{{ $data->ghichu }}</td>
                                    <td>
                                        <a type="button" class="btn bg-blue-grey waves-effect" href="{{ route('ward.edit', ['data' => $data->id]) }}">
                                            <i class="material-icons">edit</i>
                                        </a>
                                    </td>
                                    <td>
                                        <div data-toggle="modal" data-target="#cfmDel" onclick="$('#formDelete').attr('action', '{{ route('ward.destroy', ['data' => $data->id]) }}');" class="btn bg-red waves-effect">
